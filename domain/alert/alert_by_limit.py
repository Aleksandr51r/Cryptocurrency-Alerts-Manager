...
class Alert_by_limit(Alert, Cryptocurrency):
    def __init__(self, alert_id: str, type_alert: str, cryptocurrency_id: Cryptocurrency, trigger_value: float, trigger_direction: str):
        ...
        # ? Realisation of alert of changing by passing a limit
...
# * Principales methods
    ...
    def describe_alert(self):
        ...
        return f"Alert ID: {description_id(self.alert_id)} | STATUS: {description_status(status)} | Actual rate \033[44m{self.currency_rate}\033[0m | Alert will be activated if {user_choose(self.cryptocurrency.cryptocurrency_id)}'s ({user_choose(self.cryptocurrency.currency_name)}) rate will be {user_choose(self.trigger_direction)} than {user_choose(self.trigger_value)} $."
