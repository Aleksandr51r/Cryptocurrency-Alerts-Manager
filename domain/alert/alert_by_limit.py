...
# * Principales methods
    def describe_alert(self):
        ...
        return f"ALERT ID: {description_id(self.alert_id)} | STATUS: {description_status(status)} | Actual rate \033[44m{self.cryptocurrency.currency_rate}\033[0m | CONDITION: If {user_choose(self.cryptocurrency.cryptocurrency_id)}'s ({user_choose(self.cryptocurrency.currency_name)}) rate will be {user_choose(self.trigger_direction)} than {user_choose(self.trigger_value)} $."
    ...
    def is_alert_on(self):
        ...
        Args:
                self.cryptocurrency.currency_rate (float): The current price of the cryptocurrency.
        ...
        if self.trigger_direction == 'above' and self.cryptocurrency.currency_rate > self.trigger_value or self.trigger_direction == 'below' and self.cryptocurrency.currency_rate < self.trigger_value:
            self.alert_on = True
